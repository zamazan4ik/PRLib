...
void free_edge_memory(std::vector<int>& bound_list, unsigned int comp_num, unsigned char ***edge);
void  calc_boundary( marklistptr list, std::vector<int>& bound_list, unsigned char ***inner_edge,unsigned char ***outer_edge, int height, int width, unsigned char ***input_img_c, cv::Mat& bin_msk);
void calc_edge(std::vector<int>& bound_list, unsigned int comp_num, unsigned char ***inner_edge,
               unsigned char  ***outer_edge, std::vector<double>& edge_depth, std::vector<double>& edge_std,
               std::vector<double>& edge_std2, std::vector<double>& edge_max, std::vector<double>& edge_min);
...
void reverse_cc(cv::Mat& input_bin, cv::Mat& bin_img,
unsigned int height, unsigned int width, cv::Mat& flip_bin,
cv::Mat& rem_bin);
void Region_growing(CC_pixel s, cv::Mat& bin_msk, cv::Mat& out_bin, int width, int height);
...
void flip_reversed_cc( unsigned char ***input_img, cv::Mat& bin_msk, unsigned int height, unsigned int width);

void  make_feat( marklistptr list, unsigned int comp_num,
unsigned int height, unsigned int width, unsigned char ***input_img,
cv::Mat& bin_msk, double **vector);

void QuickSort(std::vector<unsigned int>& array, std::vector<unsigned int>& index,\
...
unsigned int find_percentile(std::vector<unsigned int>& data_array, int num,
